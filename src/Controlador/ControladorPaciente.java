...
public class ControladorPaciente {
    ...
    public boolean verificarEmail(String Emal) {

        String consultaPersona = "SELECT Id_Persona, prim_Nombre, seg_Nombre, prim_Apellido, seg_Apellido FROM Persona WHERE Email = ?";
        Conexion conexion = new Conexion();
        try ( Connection conn = conexion.getConexion();  PreparedStatement pers = conn.prepareStatement(consultaPersona)) {
            pers.setString(1, Emal);
            try ( ResultSet rs = pers.executeQuery()) {
                if (rs.next()) {

                    System.out.println("Existe un correo con estos datos");                   
                    return false;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public boolean registrar(Discapacidad discapacidad, Paciente paciente, AntecedentesPersonales antecedentes,
            List<AntecedentesFamiliares> familiares, int idDoctor, String rol, Estudiante estudiante, boolean identificacion) {

        
        if (verificarEmail(paciente.getEmail())) {
        ...
        if (identificacion) {
            ...
            String sqlPersona = "INSERT INTO Persona (Identificacion, prim_Nombre, seg_Nombre, prim_Apellido, seg_Apellido, Email, Direccion, Barrio, Canton, Provincia, Telefono, Fecha_Nacimiento, Lugar, Pais, Genero, Estado_Civil, Sexo, Foto, Etnia, Fecha_Registro, Contacto_Emergencia, Estado_Activo, tipo_sangre) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            String sqlEstudiante = "INSERT INTO Alumno (Id_Paciente, Id_Carrera, Alumno_Activo, Ciclo) VALUES ( ?, ?, ?, ?)";
            String sqlRoles = "INSERT INTO roles (Id_Paciente,nombreRoles) VALUES (?, ?)";
            String sqlDiscapacidad = "INSERT INTO discapacidad (D_Carnet_Conadis, D_Tipo_Discapacidad, D_Porct_Discapacidad, Observacion, Id_Persona) VALUE (?, ?, ?, ?, ?)";
            try {
...
//            
                pS.setString(21, paciente.getContactoEmergencia());
                pS.setBoolean(22, paciente.isEstadoActivo());
                pS.setString(23, paciente.getTipo_sangre());
                pS.executeUpdate();
                ...
                // Insertar en la tabla Estudiante o Roles según corresponda
                ...
                } else if ("docente".equalsIgnoreCase(rol) || "servicio".equalsIgnoreCase(rol) || "Personal Administrativo".equalsIgnoreCase(rol)) {
                    System.out.println("El rol es " + rol);
                    pS = conectBase.prepareStatement(sqlRoles, PreparedStatement.RETURN_GENERATED_KEYS);
                    pS.setInt(1, idPaciente);
                    pS.setString(2, rol);
                    pS.executeUpdate();
                } else if (rol == null) {
                ...
                if (discapacidad != null
                        && (discapacidad.getTipo_Discapacidad() != null || discapacidad.getObservacion() != null)
                        || discapacidad.getPorct_Discapacidad() != 0) {

                    pS = conectBase.prepareStatement(sqlDiscapacidad, PreparedStatement.RETURN_GENERATED_KEYS);
                    pS.setBoolean(1, discapacidad.isCarnet_Conadis());
                    pS.setString(2, discapacidad.getTipo_Discapacidad());
                    pS.setInt(3, discapacidad.getPorct_Discapacidad());
                    pS.setString(4, discapacidad.getObservacion());
                    pS.setInt(5, idPersona);

                    pS.executeUpdate();
                    System.out.println("SE GUARDO DISCAPACIDAD");

                } else {

                    System.out.println("La discapacidad es como nulo o tien datos nulos");
        ...
        Notificaciones.error("ERROR", "EL CORREO YA SE ENCUENTRA REGISTRADO");
        return false;
        }
